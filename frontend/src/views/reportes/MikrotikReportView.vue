...
onMounted(async () => {
  const { data } = await supabase.from('mikrotik_devices').select('*').order('name')
...
<template>
  <div class="mx-auto max-w-3xl px-6 py-8">
    <h1 class="mb-1 text-xl font-semibold text-slate-900">Reporte de MikroTik</h1>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-if="!devices.length">
            <td colspan="5" class="px-4 py-6 text-center text-slate-400">Sin MikroTik registrados</td>
          </tr>
