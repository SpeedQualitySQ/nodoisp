...
const exceedsStock = computed(() => !!selectedProduct.value && form.quantity > selectedProduct.value.stock)
...
async function onSubmit() {
  if (exceedsStock.value) {
    errorMsg.value = `Stock insuficiente: solo hay ${selectedProduct.value?.stock} unidad(es) disponibles.`
    return
  }
  saving.value = true
  errorMsg.value = ''
  ...
  try {
    ...
    okMsg.value = `Egreso registrado: ${form.quantity} unidad(es) de ${selectedProduct.value?.name}.`
    form.product_id = ''
    form.quantity = 1
    form.notes = ''
    await Promise.all([loadProducts(), loadMovements()])
  } catch (e) {
    errorMsg.value = e instanceof Error ? e.message : 'No se pudo registrar el egreso'
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="mx-auto max-w-3xl px-6 py-8">
    <div class="mb-6">
      <h1 class="text-xl font-semibold text-slate-900">Egreso de inventario</h1>
      <p class="text-sm text-slate-500">Registrar salida de equipos del stock (instalación en cliente)</p>
    </div>
    ...
    <form class="mb-8 grid grid-cols-2 gap-4 rounded-xl border border-slate-200 bg-white p-6" @submit.prevent="onSubmit">
      ...
      <div class="space-y-1">
        ...
        <p v-if="exceedsStock" class="text-xs text-red-600">Supera el stock disponible ({{ selectedProduct?.stock }}).</p>
      </div>
      <div class="space-y-1">
        <label class="text-sm font-medium text-slate-700">Notas</label>
        <input v-model="form.notes" placeholder="Ej. instalación cliente #123" class="input" />
      </div>
      <div class="col-span-2 flex justify-end">
        <button
          type="submit"
          :disabled="saving || !form.product_id || exceedsStock"
    ...
    <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Últimos egresos</h2>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-else-if="!movements.length">
            <td colspan="4" class="px-4 py-6 text-center text-slate-400">Sin egresos todavía</td>
          </tr>
          <tr v-for="m in movements" :key="m.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 tabular-nums font-medium text-orange-600">-{{ m.quantity }}</td>
